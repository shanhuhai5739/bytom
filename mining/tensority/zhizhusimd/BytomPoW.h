#ifndef BYTOMPOW_ZHIZHU_H
#define BYTOMPOW_ZHIZHU_H
...
struct BytomMatList16_Simd {
    ...
    void copyFrom(BytomMatList16_Simd& other) {
        for(int i=0; i<256; i++) {
            matVec[i]->copyFrom(*other.matVec[i]);
        }
    }
...
static inline void iter_mineBytom(
                        ...
                        uint32_t len,
                        // uint8_t nonce[8],
                        uint8_t result[32],BytomMatList16_Simd* matList_int16) {
